class Notepad {
  ...
  insertTextAsWordBlocks(text, options = {}) {
    ...
    words.forEach((word, index) => {
      ...
      // Respetar modo plano si está activo
  ...
  // ---------------- Modos de Inserción Planos ----------------

  setInsertionMode(mode) {
    const validModes = ['letter', 'word', 'plain-letter', 'plain-word'];
    if (validModes.includes(mode)) {
      this.opts.insertionMode = mode;
      console.log(`Modo de inserción cambiado a: ${mode}`);
    } else {
      console.warn(`Modo de inserción inválido: ${mode}. Modos válidos: ${validModes.join(', ')}`);
    }
  }
  ...
  // ---------------- Modo Texto Plano (Visualización) ----------------
  ...
  _updatePerspectiveVisual() {
    ...
    // Request: "cuanto más alejado esté el span más pequeño hasta lo mínimo que vamos a marcar como 8px"
    ...
    return new Promise((resolve, reject) => {
      ...
      try {
        ...
        for (let i = 0; i < this.letterNodes.length; i++) {
          ...
          // Determinar color para exportación
          let color;
  ...
  _attachEvents() {
    // ... (código de eventos, mantener igual que antes)
  ...
  _parseColor(colorString) {
    ...
    if (colorString.startsWith('linear-gradient(') ||
      ...
      return {
        type: 'gradient',
        value: colorString   // se enviará tal cual al estilo del span
      };
    }
