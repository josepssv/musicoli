...
// ========================================
// REVERSE FUNCTION: COLOR(S) TO MIDI
// ========================================

/**
 * Función Inversa: Convierte un array de colores (extraídos de un degradado o array)
 * de vuelta a las notas MIDI correspondientes, deshaciendo la lógica de tríos RGB.
 * "Para cualquier número de colores desde 0 a 32, obtendremos las notas de un compás completo".
 * 
 * @param {string[]} colors - Array de colores (ej. ['#ff0000', '#00ff00'])
 * @param {number} minTesitura - Rango inferior de la voz (ej. 36)
 * @param {number} maxTesitura - Rango superior de la voz (ej. 84)
 * @param {number} maxNotes - Máximo de notas permitidas en el compás (por defecto 32 fusas)
 * @returns {number[]} - Array de números MIDI reconstruidos
 */
function scaleColorsToMidiNotes(colors, minTesitura = 36, maxTesitura = 84, maxNotes = 32) {
    if (!colors || colors.length === 0) return [];

    // Validación de tesitura
    if (typeof minTesitura !== 'number' || typeof maxTesitura !== 'number' || minTesitura >= maxTesitura) {
        minTesitura = 36;
        maxTesitura = 84;
    }
    ...
    // Velocidad asumida genéricamente en la codificación original es mf (64)
    const velocityFactor = 0.3 + (64 / 127) * 1.2; // ~0.904
    ...
    colors.forEach(colorStr => {
        ...
        // 1. Decodificar el String de color a RGB
        if (colorStr.startsWith('#')) {
            const hex = colorStr.replace('#', '');
            if (hex.length === 3) { // formato corto
                r = parseInt(hex.charAt(0) + hex.charAt(0), 16);
                g = parseInt(hex.charAt(1) + hex.charAt(1), 16);
                b = parseInt(hex.charAt(2) + hex.charAt(2), 16);
            } else if (hex.length === 6) { // formato completo
        ...
        // 2. Revertir el ajuste dinámico por velocity (agrandando el valor)
        let baseR = Math.min(255, r / velocityFactor);
        let baseG = Math.min(255, g / velocityFactor);
        let baseB = Math.min(255, b / velocityFactor);

        // 3. Revertir la división del componente mínimo (FACTOR_REDUCCION_MINIMO = 2)
        // Detectamos el canal más bajo y asumimos que fue partido a la mitad.
        const currentMin = Math.min(baseR, baseG, baseB);
        let origR = baseR; let origG = baseG; let origB = baseB;

        // Tolerancia a pequeños errores de redondeo decimal
        if (Math.abs(baseR - currentMin) < 2) origR = Math.min(255, baseR * 2);
        if (Math.abs(baseG - currentMin) < 2) origG = Math.min(255, baseG * 2);
        if (Math.abs(baseB - currentMin) < 2) origB = Math.min(255, baseB * 2);

        // 4. Reconversión de componentes a Nota MIDI (Invirtiendo: val = minTes + factor * (note - minTes))
        const calculateNote = (component) => {
            if (factor === 0) return minTesitura;
            const note = minTesitura + (component - minTesitura) / factor;
            return Math.max(minTesitura, Math.min(maxTesitura, Math.round(note)));
        };

        const n1 = calculateNote(origR);
        const n2 = calculateNote(origG);
        const n3 = calculateNote(origB);

        // Añadimos las tres notas decodificadas de este color
        notes.push(n1, n2, n3);
    });

    // Limitar al número máximo de notas permitidas en el compás
...
function getMeasureVoiceColor(measureIndex, voiceKey) {
    ...
    // 2. If no voice data or no notes, return gray/black
    if (!voiceData || !voiceData.nimidi || voiceData.nimidi.length === 0) {
        return '#000000';
    }

    // 3. Check for silence based on tipis (Enforce visual silence)
    ...
    // Si por alguna razón no hay notas, aplicar fallback
    ...
    for (let i = 0; i < voiceData.nimidi.length && noteIndex < notes.length; i++) {
        ...
        if (typeof midiValue === 'number' && midiValue > 0) {
            velocities.push(allVelocities[i] || 64);
            noteIndex++;
        }
    }
...
function generateSimilarColors(h, s, l, num) {
    let colors = [];
    const hueVariation = 15; // Allow a bit of hue wobble
    ...
    for (let i = 0; i < num; i++) {
        // Slight randomization around the original center to stay affine but varied
        currentH = (h + (Math.random() * hueVariation - hueVariation / 2) + 360) % 360;
        currentS = Math.max(20, Math.min(100, s + (Math.random() * 30 - 15)));
        currentL = Math.max(20, Math.min(80, l + (Math.random() * 30 - 15)));

        const sArr = currentS / 100;
        const lArr = currentL / 100;
        const a = sArr * Math.min(lArr, 1 - lArr);
...
async function createTonalidadEditor(notepadInstance, containerId) {
    ...
    */
    ...
    // Global toggle function (kept for logic if needed, but buttons removed per user request)
    window.toggleMelodyEditor = function(target) {
        const ladder = document.getElementById('editor-tonalidad-ladder');
        const grid = document.getElementById('tonality-grid-container');
        const chords = document.getElementById('editor-chords-panel');
        
        if (target === 'scales') {
            if (ladder) ladder.style.display = 'block';
            if (grid) grid.style.display = 'grid';
            if (chords) chords.style.display = 'none';
            ...
            if (chords) {
                ...
                if (typeof window.initChordsPanel === 'function') window.initChordsPanel();
            }
        } else {
            // Default show everything in Tonalidad
            if (ladder) ladder.style.display = 'block';
            if (grid) grid.style.display = 'grid';
            if (chords) {
                chords.style.display = 'block';
    ...
    // Create container for the palette grid to toggle it
    const paletteGridContainer = document.createElement('div');
    paletteGridContainer.id = 'tonality-grid-container';
    paletteGridContainer.style.display = 'grid';
    paletteGridContainer.style.gridTemplateColumns = 'repeat(auto-fill, minmax(120px, 1fr))';
    paletteGridContainer.style.gap = '10px';
    container.appendChild(paletteGridContainer);

    // Initial state: Scales
    setTimeout(() => {
        // Set default styles for btnScales because ID might not be in DOM yet
        const bS = document.getElementById('switcher-btn-scales');
        if (bS) { bS.style.background = '#ff9800'; bS.style.color = 'white'; }
    }, 0);
    ...
    const grid = paletteGridContainer; // Use the toggleable container
    grid.style.gap = '10px';
...
function addClickHandlerScale(span, redValue, greenValue, blueValue, notes, midiPattern, direction, interval) {
    span.addEventListener('click', () => {
        ...
        // MUSICOLI: Trigger the central "Accept" logic from the rhythm editor.
        // This is the single source of truth for measure insertion (handles mode, multiplier, etc.)
        if (typeof window.triggerAcceptRhythm === 'function') {
            window.triggerAcceptRhythm();
        }
...
function createRitmoEditor(containerId) {
    ...
    // Color A info and control - REUSE FROM HTML
    const colorInfoDiv = document.getElementById('rhythm-color-info-div') || document.createElement('div');
    if (!colorInfoDiv.id) colorInfoDiv.id = 'rhythm-color-info-div';
    ...
    // MUSICOLI: Row 1 holds colorPreview + colorMelodi (Melodía items)
    ...
    // MUSICOLI: Row 2 holds notationContainer + percToggleContainer + aplicarRitmoBtn (Rhythm items)
    const colorInfoRow2 = document.createElement('div');
    window.rhythmSendPanel = colorInfoRow2; // Expose globally for persistence
    ...
    colorInfoRow2.style.marginTop = '4px'; // Added small top margin for when relocated
    colorInfoRow2.style.background = 'rgba(255,255,255,0.05)'; // Apply panel style
    colorInfoRow2.style.padding = '5px';
    colorInfoRow2.style.borderRadius = '4px';
    colorInfoRow2.style.border = '1px solid rgba(255,255,255,0.1)';
    colorInfoRow1.style.width = 'fit-content';
    ...
    window.updateRhythmColorInfoVisibility = function () {
        ...
        if (typeof currentEditMode !== 'undefined') {
            if (currentEditMode === 'tonalidad') { // Melodía
                ...
                // Restaurar visibilidad en modo ritmo solo si PERC está activo
    ...
    const populateFlavors = (char) => {
        ...
        if (flavors && Array.isArray(flavors)) {
            ...
            flavors.forEach(color => {
                ...
                // Relate MIDI values to color based on current voice
                let targetVoice = 's';
                const voiceSelector = document.getElementById('voice-selector');
                if (voiceSelector) {
                    targetVoice = voiceSelector.value;
                }

                let midiNotes = [60, 64]; // Fallback
                if (typeof rgbToMidiInterval === 'function') {
                    midiNotes = rgbToMidiInterval(r, g, b, 60, targetVoice);
                }
    ...
    toplayi.addEventListener('click', (e) => {
        ...
        if (spans.length === 0 || !window.bdi || !window.bdi.bar) return;

        // Determine selected measure range
        let startIdx = (typeof window.selectionRange !== 'undefined' && window.selectionRange.start !== -1)
            ? window.selectionRange.start
            : (typeof window.selectedMeasureIndex !== 'undefined' ? window.selectedMeasureIndex : 0);

        let endIdx = (typeof window.selectionRange !== 'undefined' && window.selectionRange.end !== -1)
            ? window.selectionRange.end
            : startIdx;

        if (startIdx < 0) startIdx = 0;
        if (endIdx >= window.bdi.bar.length) endIdx = window.bdi.bar.length - 1;
        ...
        for (let i = 0; i < selectedCount; i++) {
            let measureIndex = startIdx + i;
            if (!window.bdi.bar[measureIndex]) continue;

            // Deep clone to avoid touching original score during preview
            let measureCopy = JSON.parse(JSON.stringify(window.bdi.bar[measureIndex]));

            let spanIndex = i % spans.length;
            let span = spans[spanIndex];
            let coli = span.style.backgroundColor;

            const updateData = (data) => {
                if (!data.tipis || data.tipis.length === 0) return;

                // Re-calculate the midi notes dynamically for the specific target voice
                let newMidiTones = [];
                const rgbMatch = coli.match(/\d+/g);
                if (rgbMatch && rgbMatch.length >= 3) {
                    const r_val = parseInt(rgbMatch[0]);
                    const g_val = parseInt(rgbMatch[1]);
                    const b_val = parseInt(rgbMatch[2]);
                    if (typeof rgbToMidiInterval === 'function') {
                        newMidiTones = rgbToMidiInterval(r_val, g_val, b_val, 60, targetVoice);
                    }
                }

                // Fallback in case of failure
                if (newMidiTones.length === 0) {
                    newMidiTones = JSON.parse(span.dataset.midiValues);
                }

                let newNimidi = [];
                let noteCounter = 0;

                for (let k = 0; k < data.tipis.length; k++) {
                    if (data.tipis[k] < 0 || Math.abs(data.tipis[k]) === 100) {
                        newNimidi.push(0);
                    } else {
                        newNimidi.push(newMidiTones[noteCounter % newMidiTones.length]);
                        noteCounter++;
                    }
                }
                data.nimidi = newNimidi;
            };

            // Update only the targeted voice in the cloned measure
            if (measureCopy.voci) {
                let voiceData = null;
                if (Array.isArray(measureCopy.voci)) {
                    voiceData = measureCopy.voci.find(v => v.nami === targetVoice);
                } else {
                    voiceData = measureCopy.voci[targetVoice];
                }
                if (voiceData) updateData(voiceData);
            } else {
                updateData(measureCopy);
            }

            previewMeasures.push(measureCopy);
        }

        if (typeof window.playMeasureFast === 'function') {
            window.playMeasureFast(0, previewMeasures, [targetVoice]);
        }
    });
    colorMelodi.appendChild(toplayi);

    const toescori = document.createElement('button');
    toescori.textContent = 'Colorear';
    ...
    percInput.addEventListener('change', (e) => {
        ...
        // MUSICOLI: Toggle visibility of the "Apply Rhythm" button based on PERC state
        const aplicarBtn = document.getElementById('aplicar-ritmo-btn');
        if (aplicarBtn) {
            aplicarBtn.style.display = e.target.checked ? 'inline-flex' : 'none';
        }
    ...
    toescori.addEventListener('click', (e) => {
        ...
        const currentPat = currentPattern || [1, 1]; // Fallback to avoid error

        const spans = document.querySelectorAll('#selected-tones-container span[data-midi-values]');
        if (spans.length === 0 || !bdi || !bdi.bar) return;
        ...
        if (startIdx < 0) startIdx = 0;
        if (endIdx >= bdi.bar.length) endIdx = bdi.bar.length - 1;
        if (startIdx > endIdx) return;
        ...
        for (let i = 0; i < selectedCount; i++) {
            let measureIndex = startIdx + i;
            if (!bdi.bar[measureIndex]) continue;

            let spanIndex = i % spans.length; // cycle colors
            let span = spans[spanIndex];

            let newMidiTones = JSON.parse(span.dataset.midiValues);
            let hexi = colorToHex(span.style.backgroundColor);
            let coli = span.style.backgroundColor;
            let measure = bdi.bar[measureIndex];

            const updateData = (data) => {
                // If there's no rhythm data to color, do nothing.
                if (!data.tipis || data.tipis.length === 0) return;
                ...
                for (let k = 0; k < data.tipis.length; k++) {
                    // Check if it's a rest or a barline (which have no pitch)
                    if (data.tipis[k] < 0 || Math.abs(data.tipis[k]) === 100) {
                        newNimidi.push(0);
                    } else {
                        // It's a note, assign the next pitch from the selected color
                        newNimidi.push(newMidiTones[noteCounter % newMidiTones.length]);
                        noteCounter++;
                    }
                }

                // Only update pitch and colors, leave tipis (rhythm) untouched
                data.nimidi = newNimidi;
                data.coli = coli;
                data.hexi = hexi;

                if (!data.dinami || data.dinami.length !== newNimidi.length) {
                    data.dinami = new Array(newNimidi.length).fill(64);
                }
            };

            // Update the target voice or root measure
            if (measure.voci) {
                let voiceData = null;
                if (Array.isArray(measure.voci)) {
                    voiceData = measure.voci.find(v => v.nami === targetVoice);
                } else {
                    voiceData = measure.voci[targetVoice];
                }
                // We only color *existing* data. If the voice doesn't exist or is empty, it skips it.
                if (voiceData) updateData(voiceData);
            } else {
                updateData(measure);
            }
    ...
    acceptBtn.textContent = (typeof t === 'function') ? t("Empezar") : 'Accept';
    ...
    acceptBtn.onclick = async (e) => {
        ...
        if (bdiRef && recordiRef) {
                ...
                if (useSelectedTone) {
                    ...
                    if (container && container.children.length > 0) {
                        ...
                        toneSpans.forEach(span => {
                            ...
                            // Force monochromatic in rhythm mode
                            const isRhythmMode = (typeof window.currentEditMode !== 'undefined' && window.currentEditMode === 'ritmo') || (typeof currentEditMode !== 'undefined' && currentEditMode === 'ritmo');
                            if (isRhythmMode) {
                                spanMidi = new Array(spanMidi.length).fill(spanMidi[0]);
                            }
            ...
            // STANDARDIZED INSERTION LOGIC
            // Use selectionRange if available, fallback to selectedMeasureIndex
            const selectionRange = window.selectionRange || { start: -1, end: -1 };
            const baseIndex = (selectionRange.start !== -1) 
                ? selectionRange.start 
                : (window.selectedMeasureIndex !== -1 ? window.selectedMeasureIndex : bdiRef.length);
            
            const selectedCount = (selectionRange.start !== -1 && selectionRange.end !== -1)
                ? (selectionRange.end - selectionRange.start + 1)
                : 1;

            const cursorMeasureIndex = baseIndex;
            ...
            // Multiplier logic: Use the larger of the selection count or the manual multiplier
            const multiplierValue = (typeof rhythmMultiplierSelect !== 'undefined') ? parseInt(rhythmMultiplierSelect.value) || 1 : 1;
            const finalCount = Math.max(multiplierValue, selectedCount);

            for (let m = 0; m < finalCount; m++) {
                const currentIterationIndex = cursorMeasureIndex + m;

                if (typeof window.addMeasureWithMode === 'function') {
                    // Update behavior based on independent/dependent mode
                    const currentMode = (typeof window.voiceEditMode !== 'undefined') ? window.voiceEditMode : 'dependent';
                    const measureData = { ...activeVoiceData };
                    const voiceCode = selectedMatchCode || 's';

                    if (currentMode === 'independent') {
                        // MODO INDEPENDIENTE: Solo en la fila actual (pista activa)
                        window.addMeasureWithMode(measureData, voiceCode, currentIterationIndex);
                    } else {
                        // MODO DEPENDIENTE: Una sola llamada y addMeasureWithMode se encarga de armonizar las 4 pistas
                        window.addMeasureWithMode(measureData, voiceCode, currentIterationIndex);
                    }
                } else {
            ...
            if (typeof updateAfterBdiChange === 'function') {
                ...
                if (typeof window.selectedMeasureIndex !== 'undefined') {
                    window.selectedMeasureIndex = cursorMeasureIndex + finalCount;
                    if (window.np6) {
                        window.np6.cursorPos = cursorMeasureIndex + finalCount;
                ...
                if (typeof np6 !== 'undefined') {
                    np6.cursorPos = cursorMeasureIndex + finalCount;
                    np6._render();
                    if (typeof np6.focus === 'function') np6.focus();
                    if (typeof np6.scrollToCursor === 'function') np6.scrollToCursor();
                }
    ...
    aplicarRitmoBtn.style.cssText = `
        display: ${window.isGlobalPercussionEnabled ? 'inline-flex' : 'none'};
        align-items: center;
        gap: 4px;
        padding: 4px 10px;
        font-size: 10px;
    ...
    aplicarRitmoBtn.addEventListener('click', () => {
        ...
        // MUSICOLI: Hide the button after it's applied
        if (aplicarRitmoBtn) aplicarRitmoBtn.style.display = 'none';
    ...
    // MUSICOLI: Row 1 holds the tonality palettes (color preview etc)
    // MUSICOLI: Row 2 holds notationContainer + perc toggle + aplicar ritmo (Rhythm mode panel)
    colorInfoRow2.appendChild(notationContainer);
...
function rgbToMidiInterval(r, g, b, notaBase = 36, voiceOverride = null) {
    ...
    // Si es gris/monocromático absoluto o si estamos en el "modo ritmo", devolvemos una repetición plana
    const isMonochromatic = (r === g && g === b) || (typeof currentEditMode !== 'undefined' && currentEditMode === 'ritmo') || (typeof window.currentEditMode !== 'undefined' && window.currentEditMode === 'ritmo');
    if (isMonochromatic) {
        let normalized = (r - 50) / 205;
        let valNorm = Math.min(1, Math.max(0, normalized));
        let idx = Math.floor(valNorm * notasDisponibles.length);
        idx = Math.min(idx, notasDisponibles.length - 1);
        let note = notasDisponibles[Math.max(0, idx)];
        return [note, note, note, note, note, note, note, note];
    }

    // --- NUEVA LÓGICA: EXTRAER MAYOR VARIEDAD DE TONOS ---
    const result = [];

    // 1. Calcular propiedades del color
    const luma = (0.299 * r + 0.587 * g + 0.114 * b) / 255; // Brillo (0 a 1)
    const maxC = Math.max(r, g, b);
    const minC = Math.min(r, g, b);
    const saturation = maxC === 0 ? 0 : (maxC - minC) / maxC; // Saturación (0 a 1)

    // 2. Centro de la melodía basado en el brillo (luma)
    // Dejamos un margen del 20% en los bordes para tener espacio para saltos
    const valCentrado = 0.2 + (luma * 0.6);
    const baseIndex = Math.floor(valCentrado * notasDisponibles.length);

    // 3. Tamaño de los saltos (intervalos) basado en la saturación
    // Colores opacos = notas conjuntas (1). Colores vibrantes = grandes saltos (hasta 5 índices)
    let step = 1 + Math.floor(saturation * 4);

    // 4. Patrón y contorno basado en el color dominante
    const sequenceLen = 8; // Extraemos 8 notas para generar gran variedad
    for (let i = 0; i < sequenceLen; i++) {
        let offset = 0;

        if (r >= g && r >= b) {
            // Predomina ROJO: Patrón en Arco (Sube intenso y luego cae)
            if (i < 4) offset = i * step;
            else offset = (7 - i) * step;

        } else if (g > r && g >= b) {
            // Predomina VERDE: Patrón Zig-Zag Alterno (Sube y baja cruzando el centro)
            offset = (i % 2 === 0) ? (Math.floor(i / 2) * step) : -(Math.floor((i + 1) / 2) * step);

        } else {
            // Predomina AZUL: Patrón en cascada descendente ondulada
            offset = -(i * step) + (i % 2 === 0 ? step : 0);
            // Ajustar para centrarlo más
            offset += (sequenceLen / 2 * step);
        }

        // Aplicar el salto al centro base
        let targetIndex = baseIndex + offset;

        // "Espejismo" (Rebote) si la melodía intenta salirse del registro del instrumento
        if (targetIndex >= notasDisponibles.length) {
            let overflow = targetIndex - (notasDisponibles.length - 1);
            targetIndex = (notasDisponibles.length - 1) - overflow;
        }
        if (targetIndex < 0) {
            targetIndex = Math.abs(targetIndex);
        }

        // Doble aseguramiento de los límites
        targetIndex = Math.max(0, Math.min(targetIndex, notasDisponibles.length - 1));

        result.push(notasDisponibles[targetIndex]);
    }
...
function rgbToMidiInterval02(r, g, b, notaBase = 36) {
    ...
    const voiceSelector = document.getElementById('voice-selector');
    const voice = voiceSelector ? voiceSelector.value : 's';

    // Ajustar nota base según tesitura (aproximaciones MIDI)
    let base = notaBase;
    switch (voice) {
        case 's': base = 60; break;
        case 'a': base = 53; break;
        case 't': base = 48; break;
        case 'b': base = 40; break;
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    window.playMeasureFast = function (nc, measureOverride = null, forceVoices = null) {
        ...
        if (forceVoices) {
            voicesToPlay = Array.isArray(forceVoices) ? forceVoices : [forceVoices];
        } else {
        ...
        if (measureOverride) {
            // Use override data
            const overrideArray = Array.isArray(measureOverride) ? measureOverride : [measureOverride];
            for (let i = 0; i < overrideArray.length; i++) {
                recordi(overrideArray, i, localTracks, voicesToPlay);
            }
        } else {
  ...
  <div class="section">
    ...
    function setMode(mode) {
        ...
        const hideAllEditors = () => {
            ...
            if (chordsPanel) chordsPanel.style.display = 'none';
        };
        ...
        if (window.editMode === 'color') {
            ...
            const chordsPanel = document.getElementById('editor-chords-panel');
            if (chordsPanel) chordsPanel.style.display = 'block';
            ...
            // Sync MIDI visualizer with global toggle
            if (typeof window.toggleMidiStaff === 'function') {
                const toggle = document.getElementById('score-viz-toggle');
                if (toggle) window.toggleMidiStaff(toggle.checked);
            }
    ...
    if (empezarBtn) {
        empezarBtn.addEventListener('click', () => {
            ...
            const successText = (typeof currentLanguage !== 'undefined' && currentLanguage === 'es') ? '¡Vaciado!' : 'Emptied!';
            empezarBtn.textContent = successText;
    ...
    if (modeTonalidadBtn) {
        modeTonalidadBtn.addEventListener('click', () => {
            ...
            const editorTonalidad = document.getElementById('editor-tonalidad');
            const editorChordsPanel = document.getElementById('editor-chords-panel');
            const tonalidadLadder = document.getElementById('editor-tonalidad-ladder');

            if (editorTonalidad) {
                editorTonalidad.style.display = 'block';
                // Reset to scales mode by default when entering the mode
                if (typeof window.toggleMelodyEditor === 'function') {
                    window.toggleMelodyEditor('scales');
                } else {
                    if (tonalidadLadder) tonalidadLadder.style.display = 'block';
                    if (typeof makeladi === 'function') setTimeout(() => makeladi(), 0);
                }
    ...
    window.openMidiEditor = function (measureIndex, noteIndex = -1) {
        ...
        // MUSICOLI: Allow MIDI editor in composition mode if chord insertion is active
        ...
        if (mode === 'composicion' && !window.chordInsertModeEnabled) {
            return;
        }
        ...
        if (measureNumSpan) {
            ...
            duplicateBtn.onclick = () => {
                ...
                // 1. Clone current measure (Deep copy) directly from BDI
                ...
                // 4. Open editor for the NEW duplicate (measureIndex + 1)
                window.openMidiEditor(measureIndex + 1);
            };
            ...
            splitBtn.onclick = () => {
                ...
                window.isSplittingMeasure = true;

                // 1. Auto-accept current changes
                if (midiEditorAccept) midiEditorAccept.click();

                setTimeout(() => {
                    window.isSplittingMeasure = false;
                    ...
                    voiceOrder.forEach((activeVoiceCode) => {
                        // Clone the entire measure
                        const newMeasure = JSON.parse(JSON.stringify(currentMeasure));

                        newMeasure.voci.forEach(voice => {
                            if (voice.nami !== activeVoiceCode) {
                                // This is NOT the active voice - silence it physically.
                        ...
                        // Tell the system NOT to recalculate harmony for this measure
                        newMeasure.voicesOverride = true;

                        newMeasures.push(newMeasure);
                    });

                    // 4. Apply measures based on mode
                    const isIndependent = ((typeof window.voiceEditMode !== 'undefined' && window.voiceEditMode === 'independent') || (typeof voiceEditMode !== 'undefined' && voiceEditMode === 'independent'));

                    if (!isIndependent) {
                        // Modo dependiente: Replace current measure with the 4 new measures (desplaza el resto)
                        window.bdi.bar.splice(measureIndex, 1, ...newMeasures);
                    } else {
                        // Modo independiente: modify corresponding measures (or create if missing)
                        for (let i = 0; i < 4; i++) {
                            const targetIndex = measureIndex + i;
                            const targetVoiceCode = voiceOrder[i];
                            const sourceVoiceData = newMeasures[i].voci.find(v => v.nami === targetVoiceCode);

                            if (targetIndex < window.bdi.bar.length) {
                                // Compás existente: NO reemplazar entero.
                                // Modificar SÓLO la voz correspondiente, dejando las demás voces intocables.
                                const targetMeasure = window.bdi.bar[targetIndex];

                                // Impedir que se autoarmonice silencios de estar incompletas
                                targetMeasure.voicesOverride = true;

                                if (targetMeasure.voci && Array.isArray(targetMeasure.voci)) {
                                    const targetVoice = targetMeasure.voci.find(v => v.nami === targetVoiceCode);
                                    if (targetVoice && sourceVoiceData) {
                                        targetVoice.nimidi = [...sourceVoiceData.nimidi];
                                        targetVoice.tipis = [...sourceVoiceData.tipis];
                                        targetVoice.timis = sourceVoiceData.timis ? [...sourceVoiceData.timis] : [];
                                ...
                                // No existe compás: Añadir compás con la voz sola y demás silencios (como modo dependiente)
                                window.bdi.bar.push(newMeasures[i]);
                            }
                        }
                    }
                    ...
                    if (typeof window.applyTextLayer === 'function') window.applyTextLayer();
                    if (typeof window.renderVisualTracks === 'function') window.renderVisualTracks();
                    if (typeof window.applyNotepadColoring === 'function') window.applyNotepadColoring();
                    if (typeof window.highlightMeasure === 'function') window.highlightMeasure(measureIndex);
        ...
        // EXTRACCIÓN: Función para renderizar la partitura (calculateNotePosition is now global)
        ...
        const renderMidiScorePreview = (midiVals, rhythmVals, container) => {
            ...
            sortedTimeline.forEach((currentTime) => {
                ...
                if (isLinked) {
                    ...
                    if (intervals) {
                        // For each secondary voice, show its own notes at their own time positions
                        // with the pitch updated to the chord note from the overlapping active-voice note.
                        // This mirrors exactly the logic used by Apply (acceptBtn).
                        Object.keys(otherVoicesData).forEach(vCode => {
                            // Only show a note for this voice if it starts at currentTime
                            const vNote = otherVoicesData[vCode].find(n => n.start === currentTime);
                            if (!vNote) return;

                            const isRest = vNote.midi < 0;

                            // Find the active voice note that overlaps this secondary note's start time
                            const overlappingActive = activeNotesData.find(n => {
                                const adur = Math.abs(n.duration);
                                return currentTime >= n.start && currentTime < n.start + adur;
                            });

                            let displayMidi;
                            let isHarmonyPreview = false;
                            if (overlappingActive && activeNote) {
                                // Compute the chord note for this voice based on the overlapping active note
                                const anchorMidi = Math.abs(overlappingActive.midi);
                                const h = window.getSATBNotesForChord(anchorMidi, intervals, activeVoiceCode);
                                displayMidi = isRest ? -h.satb[vCode] : h.satb[vCode];
                                isHarmonyPreview = true;
                            } else {
                                // No active note overlaps → show current BDI pitch unchanged
                                displayMidi = vNote.midi;
                            }
                            ...
                            } else if (window.chordsVerticalDynamicsEnabled && activeNote) {
                                const activeOffset = offsets[activeVoiceCode] || 0;
                                const activeDynami = (window.currentEditingDynamicsValues && window.currentEditingDynamicsValues[activeNote.index]) || 80;
                                const columnMean = activeDynami - activeOffset;
                                intensityVal = Math.max(16, Math.min(127, columnMean + (offsets[vCode] || 0)));
                            }

                            voiceNotesAtPosition.push({
                                midi: Math.abs(displayMidi),
                                voice: vCode,
                                isActive: false,
                                isHarmonyPreview,
                                intensity: intensityVal,
                                rhythm: vNote.duration
                            });
                        });

                        // Also update active voice intensity display
                        if (activeNote && voiceNotesAtPosition.length > 0) {
                        ...
                        // Link ON but no chord selected → show BDI notes as-is
                        Object.keys(otherVoicesData).forEach(vCode => {
                            const vNote = otherVoicesData[vCode].find(n => n.start === currentTime);
                            if (vNote) {
                                voiceNotesAtPosition.push({ 
                    ...
                    // Not linked → show all BDI notes starting at this time
                    Object.keys(otherVoicesData).forEach(vCode => {
                        const vNote = otherVoicesData[vCode].find(n => n.start === currentTime);
                        if (vNote) {
                            voiceNotesAtPosition.push({ 
                ...
                voiceNotesAtPosition.forEach(vNote => {
                    ...
                    let vNoteOpacity = vNote.isActive ? (isSelected ? '1' : '0.8') : (vNote.isHarmonyPreview ? '0.4' : '0.15');
        ...
        // EXPOSURE: Global refresh method for Chords Panel coordination
        window.refreshMidiEditorScore = () => {
        ...
        rhythmContainer.style.cssText = `
            ...
            background: #fff; padding: 0; height: 95px; overflow-y: hidden;
        ...
        unifiedControls.style.cssText = 'display: flex; flex-wrap: wrap; gap: 8px; padding: 6px; background: #f5f5f5; align-items: center; justify-content: flex-start; min-height: 40px; margin-top: 5px;';
        ...
        // MUSICOLI: Removed immediate append to layout them into a single line at the end
        ...
        rightGroup.id = 'midi-editor-right-group';
        ...
        // Style and Append
        if (addBtn) {
            addBtn.style.display = 'inline-flex'; // Forced visible
        ...
        if (acceptBtn) {
            acceptBtn.style.display = 'inline-flex'; // Forced visible
        ...
        if (addBtn) {
            addBtn.onclick = () => {
                ...
                // Recalculate timis (cumulative sum of durations)
                let newTimis = [];
                let currentPos = 0;
                newTipis.forEach(t => {
                    newTimis.push(currentPos);
                    currentPos += Math.abs(t);
                });
                    ...
                    if (window.voiceEditMode === 'dependent') {
                        ...
                        newMeasure.voci.forEach(v => {
                            if (v.nami !== voiceCode) {
                                ...
                                // In dependent mode, sync rhythm (tipis/timis) and dynamics count from active voice
                                v.tipis = (exV && exV.tipis && exV.tipis.length > 0) ? [...exV.tipis] : [...activeVoice.tipis];
                                
                                // Calculate correct cumulative timis for this voice
                                let voiceTimis = [];
                                let vPos = 0;
                                v.tipis.forEach(t => {
                                    voiceTimis.push(vPos);
                                    vPos += Math.abs(t);
                                });
                                v.timis = voiceTimis;
                                v.nimidi = (exV && exV.nimidi && exV.nimidi.length > 0) ? [...exV.nimidi] : new Array(v.tipis.length).fill(60);
                                v.dinami = (exV && exV.dinami && exV.dinami.length === v.tipis.length) ? [...exV.dinami] : new Array(v.tipis.length).fill(64);
                                v.ligi = (exV && exV.ligi && exV.ligi.length === v.tipis.length) ? [...exV.ligi] : new Array(v.tipis.length).fill(0);
                                v.liri = (exV && exV.liri) ? [...exV.liri] : [];

                                if (isChordLinked && chordIntervals) {
                                    // Linked Chord Harmony: Use each note's own timing to find the overlapping active voice note
                                    v.nimidi = v.nimidi.map((targetNote, vi) => {
                                        ...
                                        if (activeIdx === -1) return targetNote; 
                                        const anchorNote = activeVoice.nimidi[activeIdx];
                                        const isRest = targetNote < 0 || v.tipis[vi] < 0;
                                        const h = window.getSATBNotesForChord(Math.abs(anchorNote), chordIntervals, voiceCode);
                                        return isRest ? -h.satb[v.nami] : h.satb[v.nami];
                                    });

                                    // Apply Vertical Dynamics if enabled
                                    ...
                                    // Fallback: Rests if nothing else
                                    if (!exV) v.tipis = v.tipis.map(t => -Math.abs(t));
                                }
                            }
                        });
                    } else {
        ...
        if (acceptBtn) {
            acceptBtn.onclick = () => {
                ...
                if (window.currentEditingRhythmValues && window.currentEditingRhythmValues.length === isRestArray.length) {
                    ...
                    let mTimis = [];
                    let mPos = 0;
                    newMeasure.tipis.forEach(t => {
                        mTimis.push(mPos);
                        mPos += Math.abs(t);
                    });
                    newMeasure.timis = mTimis;
                } else {
                ...
                if (voiceEditMode === 'dependent' && !window.isSplittingMeasure) {
                    // MUSICOLI: Support for Chords Panel "Link" mode
                    const isChordLinked = !!window.chordsMidiLinkEnabled;
                    const stagedChordName = isChordLinked ? document.getElementById('chord-staff-container')?.dataset.stagedChord : null;
                    const chordIntervals = stagedChordName ? acordesNotas[stagedChordName] : null;
                        ...
                        if (v.nami !== voiceCode) {
                            ...
                            if (existingMeasureIdx >= 0 && window.bdi.bar && window.bdi.bar[existingMeasureIdx]) {
                                ...
                                if (existingMeasure.voci) {
                                    ...
                                    else exV = existingMeasure.voci[v.nami];
                                }
                            }
                            ...
                            if (isChordLinked && chordIntervals) {
                                // Linked Chord Harmony: Preserve rhythm, update pitches by mapping time
                                v.nimidi = v.nimidi.map((targetNote, vi) => {
                                    const startTime = v.timis[vi];
                                    const activeIdx = activeVoice.timis.findIndex((at, ai) => {
                                        const adur = Math.abs(activeVoice.tipis[ai]);
        ...
        genControls.style.cssText = 'display: flex; flex-wrap: nowrap; gap: 2px; align-items: center; margin: 0; padding: 0; background: transparent;';
    ...
    function addMeasureWithMode(measureData, voiceCode = null, targetIndex = -1) {
        ...
        if (voiceEditMode === 'dependent') {
            // MODO DEPENDIENTE: Inserción con desplazamiento y armonía automática
            // Siempre inserta (splice) en el lugar indicado, desplazando lo existente
            const newMeasure = {
                voci: [
            ...
            // Asegurar etiquetas de voz
            newMeasure.voci.forEach((v, idx) => v.nami = ['s', 'a', 't', 'b'][idx]);

            // Insertar (splice)
            bdi.bar.splice(measureIndex, 0, newMeasure);

        } else {
            // MODO INDEPENDIENTE: Sustitución en la pista actual, sin armonía
            const activeVoiceIdx = ['s', 'a', 't', 'b'].indexOf(voiceCode);

            if (measureIndex < bdi.bar.length) {
                // Existe el compás: Sustituir solo en la pista actual (como pidió el usuario)
                const existingMeasure = bdi.bar[measureIndex];
                if (!existingMeasure.voci) {
                ...
                if (activeVoiceIdx >= 0) {
                    existingMeasure.voci[activeVoiceIdx] = { nami: voiceCode, ...measureData };
                }

                // Sincronizar datos raíz para visualización
                existingMeasure.nimidi = [...measureData.nimidi];
                existingMeasure.tipis = [...measureData.tipis];
                existingMeasure.timis = [...measureData.timis];
            } else {
                // No existe (final de composición): Insertar con silencio en las otras pistas
                const newMeasure = {
                    voci: [
                        { nami: 's', ...createEmptyMeasure() },
                        { nami: 'a', ...createEmptyMeasure() },
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    function switchMode(mode) {
        ...
        const scoreToggle = document.getElementById('global-score-toggle');
        ...
        if (mode === 'tonalidad') {
            ...
            setTimeout(() => {
                const currentContainer = document.getElementById('harmonize-container');
                if (currentContainer) {
        ...
        const tonalidadLadder = document.getElementById('editor-tonalidad-ladder');
        if (tonalidadLadder) {
            tonalidadLadder.style.display = (mode === 'ritmo' || mode === 'tonalidad') ? 'block' : 'none';
        }
...
window.auditionSATBChord = function(rootMidi, intervals, activeVoiceCode) {
    ...
    // Determine arpeggio settings based on selected note or global UI state
    let arpeggioIntensity = window.currentEditingStagger || 0;
    let arpeggioDir = window.currentEditingStaggerDir || 1;
    
    const measureIndex = window.currentEditingMeasureIndex;
    const selectedIndices = window.selectedNoteIndices || [];
    
    if (measureIndex >= 0 && selectedIndices.length > 0 && window.bdi && window.bdi.bar) {
        const measure = window.bdi.bar[measureIndex];
        if (measure && measure.staggerNotes && measure.staggerNotes[selectedIndices[0]]) {
            const config = measure.staggerNotes[selectedIndices[0]];
            arpeggioIntensity = config.intensity !== undefined ? config.intensity : arpeggioIntensity;
            arpeggioDir = config.dir !== undefined ? config.dir : arpeggioDir;
        }
    }

    // Create a dummy measure to play via the MIDI player
    const pitches = result.satb;
    const dummyMeasure = {
        voci: [
            { nami: 's', nimidi: [pitches['s']], tipis: [4000], timis: [1], dinami: [100] },
            { nami: 'a', nimidi: [pitches['a']], tipis: [4000], timis: [1], dinami: [100] },
            { nami: 't', nimidi: [pitches['t']], tipis: [4000], timis: [1], dinami: [100] },
            { nami: 'b', nimidi: [pitches['b']], tipis: [4000], timis: [1], dinami: [100] }
        ],
        // Apply the arpeggio setting to the only note (index 0) in the dummy measure
        staggerNotes: {
            0: { intensity: arpeggioIntensity, dir: arpeggioDir }
        }
    };

    // Play all SATB voices together using the application's fast MIDI player
    if (typeof window.playMeasureFast === 'function') {
        window.playMeasureFast(0, dummyMeasure, ['s', 'a', 't', 'b']);
    } else {
        console.warn("MIDI player function playMeasureFast not available.");
    }
};
...
window.calculateNotePosition = function(midiNote) {
    ...
    return (referencePosition - absolutePosition) * pixelsPerStep;
};

/**
 * Get the exact MIDI notes for a chord distributed across SATB voices,
 * strictly enforcing vocal ranges (tessituras) while anchoring to the active voice.
 */
 ...
 * strictly enforcing vocal ranges (tessituras) while anchoring to the active voice.
 */
window.getSATBNotesForChord = function(rootMidi, intervals, activeVoiceCode = 's') {
    const intervalsCopy = [...intervals];
    
    // Mapping structure for 3 or 4+ notes
    let mapping = {
        's': intervalsCopy[2] !== undefined ? intervalsCopy[2] : intervalsCopy[intervalsCopy.length - 1],
        'a': intervalsCopy[1] !== undefined ? intervalsCopy[1] : intervalsCopy[0],
        't': intervalsCopy[0],
        'b': (intervalsCopy[0] - 12)
    };
    
    if (intervalsCopy.length >= 4) {
        mapping['s'] = intervalsCopy[3];
        mapping['a'] = intervalsCopy[2];
        mapping['t'] = intervalsCopy[1];
        mapping['b'] = intervalsCopy[0] - 12;
    }

    // Anchor note offset: The rootMidi provided IS the pitch for activeVoiceCode.
    // We shift the entire chord so that mapping[activeVoiceCode] matches rootMidi.
    const anchorOffset = mapping[activeVoiceCode] || 0;
    const chordRootMIDI = rootMidi - anchorOffset;

    const satb = {};
    Object.keys(window.globalTessituras).forEach(vCode => {
        let pitch = chordRootMIDI + mapping[vCode];
        
        // Ensure all voices (including the anchor) stick to their tessituras
        const range = window.globalTessituras[vCode];
        let safety = 0;
        while (pitch < range.min && safety < 10) { pitch += 12; safety++; }
        safety = 0;
        while (pitch > range.max && safety < 10) { pitch -= 12; safety++; }
        
        satb[vCode] = pitch;
    });
    
    return { satb, root: chordRootMIDI };
};

/**
 * Render a chord on a small 10-line staff preview.
 */
window.renderChordPreviewOnStaff = function(chordName, intervals, container) {
    if (!container) return;
    container.innerHTML = '';
    
    // Determine root note for preview
    let rootMidi;
    const selectedIndices = window.selectedNoteIndices || [];
    
    // Get current active voice to use as anchor
    const voiceSelector = document.getElementById('voice-selector');
    const activeVoiceCode = voiceSelector ? voiceSelector.value : 's';

    if (selectedIndices.length > 0 && window.currentFullMidiValues) {
        rootMidi = Math.abs(window.currentFullMidiValues[selectedIndices[0]]) || 60;
    } else {
        const baseKey = (typeof keyinselecti !== 'undefined') ? keyinselecti : 0;
        const defaults = { 
            's': window.chordsSopranoRefMidi || (72 + baseKey), 
            'a': 60 + baseKey, 
            't': 60 + baseKey, 
            'b': 48 + baseKey 
        };
        rootMidi = defaults[activeVoiceCode] || (72 + baseKey);
    }
    
    // Get SATB distribution anchored to the active voice note
    const result = window.getSATBNotesForChord(rootMidi, intervals, activeVoiceCode);
    const satbNotes = result.satb;
    
    // Actualización del nombre encima del pentagrama basado en la raíz calculada
    const nameLabel = document.getElementById('chord-preview-name-label');
    if (nameLabel) {
        const notesArr = ["C", "C#", "D", "D#", "E", "F", "F#", "G", "G#", "A", "A#", "B"];
        const rootIndex = ((result.root % 12) + 12) % 12;
        const rootName = notesArr[rootIndex];
        nameLabel.textContent = `${rootName} ${chordName}`;
    }

    // Creating the 10 lines Staff
    const staffWrapper = document.createElement('div');
    staffWrapper.style.cssText = 'position: relative; width: 100%; height: 100%; background: #fff; overflow: visible;';
    
    // Clef & Lines Helper
    const drawClefStaff = (top, glyph, glyphTop) => {
        const staff = document.createElement('div');
        staff.style.cssText = `position: absolute; top: ${top}px; left: 5px; right: 5px; height: 24px; pointer-events: none;`;
        for (let i = 0; i < 5; i++) {
            const line = document.createElement('div');
            line.style.cssText = `position: absolute; top: ${i * 6}px; left: 0; right: 0; height: 1px; background: #333;`;
            staff.appendChild(line);
        }
        staffWrapper.appendChild(staff);

        const clef = document.createElement('div');
        clef.innerHTML = glyph;
        clef.style.cssText = `position: absolute; left: 8px; top: ${glyphTop}px; font-family: "Bravura"; font-size: 24px; color: #000; z-index: 2;`;
        staffWrapper.appendChild(clef);
    }

    drawClefStaff(16, '&#xE050;', -14); // Treble Clef
    drawClefStaff(52, '&#xE062;', 10);  // Bass Clef

    // Render notes
    const notesContainer = document.createElement('div');
    notesContainer.style.cssText = 'position: absolute; inset: 0; display: flex; align-items: center; justify-content: center; padding-left: 35px;';
    staffWrapper.appendChild(notesContainer);
    ...
    // Render each SATB note
    Object.keys(satbNotes).forEach(vKey => {
        const midiVal = satbNotes[vKey];
        const intensity = chordDynamics[vKey] || 80;
        
        const noteWrapper = document.createElement('div');
        noteWrapper.style.cssText = 'position: relative; width: 0; height: 100%;';
        
        const verticalOffset = calculateNotePosition(midiVal);
        const topPos = verticalOffset * 0.5 + 19;
        
        // MUSICOLI: Unified dynamic color logic (thermal scale)
        const getDynamicColor = (val) => {
            if (val > 80) {
                const ratio = (val - 80) / (127 - 80);
                return `rgb(${Math.max(0, Math.min(255, Math.round(200 + ratio * 55)))}, ${Math.max(0, Math.min(255, Math.round(160 - ratio * 80)))}, 0)`;
            } else if (val < 80) {
                const ratio = (80 - val) / (80 - 16);
                return `rgb(${Math.max(0, Math.min(255, Math.round(80 - ratio * 80)))}, ${Math.max(0, Math.min(255, Math.round(150 - ratio * 50)))}, 255)`;
            } else {
                return '#555';
            }
        };

        const noteColor = getDynamicColor(intensity);
        
        // Note head
        const head = document.createElement('div');
        head.innerHTML = '&#xE0A4;'; 
        head.style.cssText = `position: absolute; top: ${topPos}px; left: 0; font-family: "Bravura"; font-size: 24px; color: ${noteColor}; text-shadow: 1px 1px 0px rgba(0,0,0,0.15); z-index: 10;`;
        
        // Sharp
        if ([1, 3, 6, 8, 10].includes(midiVal % 12)) {
            const sharp = document.createElement('div');
            sharp.innerHTML = '&#xE262;';
            sharp.style.cssText = `position: absolute; top: 0px; left: -14px; font-family: "Bravura"; font-size: 24px; color: ${noteColor}; text-shadow: 1px 1px 0px rgba(0,0,0,0.15);`;
            head.appendChild(sharp);
        }

        // Ledger lines
        const linesToDraw = [];
        if (midiVal === 60 || midiVal === 61) linesToDraw.push(60);
        [81, 84, 88, 91, 102].forEach(l => { if (midiVal >= l) linesToDraw.push(l); });
        [40, 36, 33, 29, 23].forEach(l => { if (midiVal <= l) linesToDraw.push(l); });

        linesToDraw.forEach(lineMidi => {
            const lOffset = calculateNotePosition(lineMidi);
            const lTop = lOffset * 0.5 + 19;
            const ledger = document.createElement('div');
            ledger.innerHTML = '&#xE022;';
            // Keeping ledger lines black for readability
            ledger.style.cssText = `position: absolute; top: ${lTop}px; left: -3px; font-family: "Bravura"; font-size: 24px; color: #000; pointer-events: none;`;
            noteWrapper.appendChild(ledger);
        });

        noteWrapper.appendChild(head);
        notesContainer.appendChild(noteWrapper);
    });
...
window.generateHarmonyForVoice = function (sourceData, voiceCode, selectedVoiceCode) {
    ...
    // MUSICOLI: Check if Chords Panel is Linked to override harmony
    if (window.chordsMidiLinkEnabled) {
        const staffContainer = document.getElementById('chord-staff-container');
        const chordName = staffContainer ? staffContainer.dataset.stagedChord : null;
        if (chordName && typeof acordesNotas !== 'undefined' && acordesNotas[chordName]) {
            const intervals = acordesNotas[chordName];
            const selectedIndices = window.selectedNoteIndices || [];
            
            // "Replicarlas": Caluclate the reference SATB notes for the first selected note
            let refPitch = null;
            let refSATB = null;
            if (selectedIndices.length > 0) {
                refPitch = Math.abs(sourceNimidi[selectedIndices[0]]) || 60;
                refSATB = window.getSATBNotesForChord(refPitch, intervals, selectedVoiceCode).satb;
            }

            let linkedNimidi = sourceNimidi.map((n, idx) => {
                const isSelected = selectedIndices.includes(idx);
                // If notes are selected but this one is NOT one of them, skip and use fallback
                if (selectedIndices.length > 0 && !isSelected) {
                    return null; // Force fallback below
                }

                // Either no selection (apply to all) or this note is selected: compute chord note
                if (typeof window.getSATBNotesForChord === 'function') {
                    const currentPitch = Math.abs(n) || 60;
                    if (selectedIndices.length > 0 && refSATB && refPitch !== null) {
                        // Apply identical intervals relative to first selected note's voicing
                        const diff = currentPitch - refPitch;
                        return refSATB[voiceCode] + diff;
                    } else {
                        // No selection: compute chord note fresh for each note
                        const satbResult = window.getSATBNotesForChord(currentPitch, intervals, selectedVoiceCode);
                        if (satbResult && satbResult.satb && satbResult.satb[voiceCode] !== undefined) {
                            return satbResult.satb[voiceCode];
                        }
                    }
                }
                return n; // Fallback to unison if generation fails
            });

            // If some notes were skipped (not selected), we must fill them from regular BDI data or scale logic
            const anyNull = linkedNimidi.includes(null);
            if (anyNull) {
                // Get pre-existing data for this voice to fill non-selected gaps
                const measureIndex = window.currentEditingMeasureIndex;
                let originalVoiceNimidi = [];
                if (measureIndex >= 0 && window.bdi && window.bdi.bar[measureIndex]) {
                    const targetV = window.bdi.bar[measureIndex].voci.find(v => v.nami === voiceCode);
                    if (targetV) originalVoiceNimidi = targetV.nimidi || [];
                }

                linkedNimidi = linkedNimidi.map((val, idx) => {
                    if (val !== null) return val;
                    // Try to preserve existing note if available, else use fallback
                    if (originalVoiceNimidi[idx] !== undefined) return originalVoiceNimidi[idx];
                    return sourceNimidi[idx]; // Absolute fallback to unison
                });
            }

            return { nimidi: linkedNimidi };
        }
    }
...
window.initChordsPanel = function() {
    ...
    // Configuración del fondo del panel de acordes (Dinámico según estado de vinculación)
    if (window.chordsMidiLinkEnabled === undefined) window.chordsMidiLinkEnabled = false;
    const bodyColorUnlinked = '#bbbbbb';
    const bodyColorLinked = '#fff';
    const headerColorUnlinked = '#bbbbbb'; // Igual que el cuerpo cuando está desvinculado
    const headerColorLinked = '#f5f5f5';

    chordsPanel.style.setProperty('background', window.chordsMidiLinkEnabled ? bodyColorLinked : bodyColorUnlinked, 'important');

    // Reset layout
    chordsPanel.innerHTML = '';
    
    // Create Layout Structure: Header, then Content
    const mainContainer = document.createElement('div');
    mainContainer.style.display = 'flex';
    mainContainer.style.flexDirection = 'column';
    mainContainer.style.width = '100%';
    
    // Create Header to match MIDI Editor
    const headerContainer = document.createElement('div');
    headerContainer.className = 'chords-header';
    headerContainer.style.cssText = 'display: flex; justify-content: space-between; align-items: center; padding: 6px 10px; border-bottom: 1px solid #ddd; border-radius: 8px 8px 0 0; margin-bottom: 10px; transition: border-radius 0.3s ease;';
    headerContainer.style.background = window.chordsMidiLinkEnabled ? headerColorLinked : headerColorUnlinked;
    
    // Set initial pegado state
    if (window.chordsMidiLinkEnabled) {
        chordsPanel.classList.add('linked-mode');
    } else {
        chordsPanel.classList.remove('linked-mode');
    }
    
    const headerTitle = document.createElement('h3');
    // Título en inglés por defecto, traducible
    headerTitle.textContent = (typeof t === 'function' ? t('Chords') : 'Chords') || 'Chords';
    // Ajustado a la derecha
    headerTitle.style.cssText = 'margin: 0; font-family: monospace; color: #333; cursor: default; user-select: none; font-size: 14px; flex: 0 0 auto; padding-right: 10px; padding-left: 10px;';
    
    // Create an empty spacer to push everything to the right
    const headerSpacer = document.createElement('div');
    headerSpacer.style.flex = '1';
    
    // --- STRUM / ARPEGGIO CONTROLS ---
    const strumGroup = document.createElement('div');
    strumGroup.style.cssText = 'display: inline-flex; align-items: center; gap: 4px; border: 1px solid #ccc; padding: 2px 4px; border-radius: 4px; background: #eceff1; margin-right: 4px; margin-left: 0px; height: 24px; box-sizing: border-box;';
    ...
    strumInp.min = 0;
    strumInp.max = 127;
    strumInp.title = (typeof t === 'function' ? t('StrumTitle') : 'StrumTitle');
    ...
    strumInp.oninput = updateStrumBrush;
    ...
    // MUSICOLI: Fixed Intensity Buttons (5, 10, 20, 40)
    [5, 10, 20, 40].forEach(val => {
        const fBtn = document.createElement('button');
        fBtn.textContent = val;
        fBtn.title = `Set arpeggio to ${val} ticks`;
        fBtn.style.cssText = 'background: #fff; color: #333; border: 1px solid #999; border-radius: 2px; cursor: pointer; font-size: 10px; font-weight: bold; height: 18px; min-width: 18px; padding: 0 2px; line-height: 1; margin: 0;';
        fBtn.onclick = (e) => {
            e.stopPropagation();
            strumInp.value = val;
            updateStrumBrush();
    ...
    linkLabelText.className = 'switch-label';
    ...
    linkCheckbox.id = 'chords-midi-link-checkbox';
    if (window.chordsMidiLinkEnabled === undefined) window.chordsMidiLinkEnabled = false;
    linkCheckbox.checked = window.chordsMidiLinkEnabled;
    
    const linkSlider = document.createElement('span');
    linkSlider.className = 'slider round';
    
    linkSwitchControl.appendChild(linkCheckbox);
    linkSwitchControl.appendChild(linkSlider);
    
    // Build the switch container
    linkContainer.appendChild(linkLabelText);
    linkContainer.appendChild(linkSwitchControl);
    
    headerContainer.appendChild(linkContainer);
    
    // Dynamics Row (Ordering: Static -> Shapes -> Mixer)
    const dynContainer = document.createElement('div');
    dynContainer.id = 'chords-dynamics-row';
    dynContainer.style.cssText = 'display: flex; gap: 2px; align-items: center; justify-content: flex-start;';
    dynContainer.style.display = 'flex';

    const dynLabels = [
        { label: 'ppp', val: 16 }, { label: 'pp', val: 33 }, { label: 'p', val: 49 },
        { label: 'mp', val: 64 }, { label: 'mf', val: 80 }, { label: 'f', val: 96 },
        { label: 'ff', val: 112 }, { label: 'fff', val: 127 }
    ];

    // 1. Add Shapes (at the beginning of the row)
    const shapeConfigs = [
        { label: '< cresc', type: 'cresc' },
        { label: '> dim', type: 'dim' },
        { label: '∧ Peak', type: 'cima' },
        { label: '∨ Valley', type: 'valle' }
    ];

    dynLabels.forEach(dyn => {
        const dBtn = document.createElement('button');
        dBtn.className = 'theme-btn compact-dyn';
        dBtn.textContent = dyn.label; // Label remains for the button
        dBtn.style.cssText = 'display: inline-flex; align-items: center; justify-content: center; padding: 5px 4px 0 4px; font-size: 11px; font-family: "Bravura", serif; font-style: italic; cursor: pointer; border: 1px solid #ccc; border-radius: 3px; background: #fff; line-height: 1; min-width: 24px; height: 18px; overflow: hidden;';
        
        dBtn.onclick = (e) => {
            e.preventDefault();
            const val = dyn.val;
            
            // MUSICOLI: Master Sync - Link ON
            if (window.chordsMidiLinkEnabled && window.chordVoiceOverwrites) {
                ['s', 'a', 't', 'b'].forEach(vK => {
                    window.chordVoiceOverwrites[vK] = val;
                });
                const mixerBtns = document.getElementById('chords-vertical-mixer')?.querySelectorAll('[id^="chords-display-"]');
                if (mixerBtns) {
                    mixerBtns.forEach((mDisplay, idx) => {
                        const vKey = ['s', 'a', 't', 'b'][idx];
                        mDisplay.textContent = vKey.toUpperCase() + ": " + dyn.label;
                        mDisplay.style.background = getDisplayColor(val);
                        mDisplay.style.color = val === 80 ? "#fff" : (val > 80 ? "#331100" : "#ffffff"); 
                        mDisplay.style.fontWeight = "bold";
                    });
                }
                if (window.refreshMidiEditorScore) window.refreshMidiEditorScore();
            } else {
                // Link OFF: Affect ONLY current Chord Staging/Preview
                if (window.chordVoiceOverwrites) {
                    ['s', 'a', 't', 'b'].forEach(vK => {
                        window.chordVoiceOverwrites[vK] = val;
                        // Update Mixer buttons/displays UI
                        const mRow = document.getElementById('chords-vertical-mixer');
                        const mixerBtns = mRow ? mRow.querySelectorAll('[id^="chords-display-"]') : null;
                        if (mixerBtns) {
                            const vIdx = ['s', 'a', 't', 'b'].indexOf(vK);
                            if (mixerBtns[vIdx]) {
                                // Updated text writing according to user request: Voice + Dynamic name
                                mixerBtns[vIdx].textContent = vK.toUpperCase() + ": " + dyn.label;
                                mixerBtns[vIdx].style.background = getDisplayColor(val); // Dynamic color
                                mixerBtns[vIdx].style.color = (val === 80 || val === null) ? "#fff" : (val > 80 ? "#331100" : "#ffffff"); 
                                mixerBtns[vIdx].style.fontWeight = "bold";
                            }
                        }
                    });
                }
                ...
                const staged = staffCt?.dataset.stagedChord;
                if (staged && acordesNotas[staged]) {
                    window.renderChordPreviewOnStaff(staged, acordesNotas[staged], staffCt);
                }
            }
            dBtn.style.background = '#e3f2fd';
            setTimeout(() => dBtn.style.background = '#fff', 200);
        };
        dynContainer.appendChild(dBtn);
    });

    // Separator between static and shapes
    const shapeSep = document.createElement('div');
    shapeSep.style.cssText = 'width: 1px; height: 16px; background: #ccc; margin: 0 4px;';
    dynContainer.appendChild(shapeSep);

    // 2. Add Shapes (Cresc, Dim, etc.)
    shapeConfigs.forEach(shape => {
        const sBtn = document.createElement('button');
        sBtn.className = 'theme-btn compact-dyn';
        sBtn.textContent = shape.label;
        sBtn.style.cssText = 'display: inline-flex; align-items: center; justify-content: center; padding: 4px 4px 0 4px; font-size: 10px; font-family: "Bravura", serif; font-style: italic; cursor: pointer; border: 1px solid #bbb; border-radius: 3px; background: #fdfdfd; line-height: 1; min-width: 28px; height: 18px; overflow: hidden;';
        
        sBtn.onclick = (e) => {
            e.preventDefault();
            
            const verticalProfiles = {
                 'cresc': { s: 112, a: 96, t: 80, b: 64 }, 
                 'dim':   { s: 64, a: 80, t: 96, b: 112 },
                 'cima':  { s: 64, a: 112, t: 112, b: 64 },
                 'valle': { s: 112, a: 64, t: 64, b: 112 } 
            };
            const profile = verticalProfiles[shape.type];

            if (window.chordsMidiLinkEnabled) {
                if (profile && window.chordVoiceOverwrites) {
                    ['s','a','t','b'].forEach(vK => {
                        const val = profile[vK];
                        window.chordVoiceOverwrites[vK] = val;
                        const mixerBtns = document.getElementById('chords-vertical-mixer')?.querySelectorAll('[id^="chords-display-"]');
                        if (mixerBtns) {
                            const idx = ['s', 'a', 't', 'b'].indexOf(vK);
                            const dLabel = dynLabels.find(dl => dl.val === val)?.label || "--";
                            if (mixerBtns[idx]) {
                                const val = profile[vK];
                                mixerBtns[idx].textContent = vK.toUpperCase() + ": " + dLabel; 
                                mixerBtns[idx].style.background = getDisplayColor(val);
                                mixerBtns[idx].style.color = val === 80 ? "#fff" : (val > 80 ? "#331100" : "#ffffff"); 
                                mixerBtns[idx].style.fontWeight = "bold";
                            }
                        }
                    });
                }
                ...
                if (profile && window.chordVoiceOverwrites) {
                    ['s','a','t','b'].forEach(vK => {
                        ...
                        if (mixerBtns) {
                            ...
                            if (mixerBtns[vIdx]) {
                                const dLabel = dynLabels.find(dl => dl.val === val)?.label || "--";
                                mixerBtns[vIdx].textContent = vK.toUpperCase() + ": " + dLabel;
                                mixerBtns[vIdx].style.background = getDisplayColor(val);
                                mixerBtns[vIdx].style.color = (val === 80 || val === null) ? "#fff" : (val > 80 ? "#331100" : "#ffffff");
                                mixerBtns[vIdx].style.fontWeight = "bold";
                            }
                        }
            ...
            sBtn.style.background = '#e3f2fd';
            setTimeout(() => sBtn.style.background = '#fdfdfd', 200);
        };
        dynContainer.appendChild(sBtn);
    });

    // We'll append dynContainer to the headerContainer later
    
    // NEW: Vertical Balance Mixer (S, A, T, B offsets)
    const mixerContainer = document.createElement('div');
    mixerContainer.id = 'chords-vertical-mixer';
    mixerContainer.style.cssText = 'display: flex; gap: 4px; padding: 4px 15px; border-bottom: 1px solid #ddd; background: inherit; align-items: center; justify-content: flex-start;';
    // Siempre visible
    mixerContainer.style.display = 'flex';

    if (window.chordVoiceOverwrites === undefined) window.chordVoiceOverwrites = { s: 80, a: 80, t: 80, b: 80 };

    // Order: 1. Dynamics Row (ppp..fff + shapes), then 2. Separator, then 3. Voice Displays
    mixerContainer.appendChild(dynContainer);
    
    // Separator between global dynamics and individual mixer
    const mixerSep = document.createElement('div');
    mixerSep.style.cssText = 'width: 1px; height: 16px; background: #ddd; margin: 0 4px;';
    mixerContainer.appendChild(mixerSep);
    
    const getDisplayColor = (vVal) => {
         if (vVal === null || vVal === undefined) return "white";
         if (vVal > 80) {
              const ratio = (vVal - 80) / (127 - 80);
              return `rgb(${Math.max(0, Math.min(255, Math.round(200 + ratio * 55)))}, ${Math.max(0, Math.min(255, Math.round(160 - ratio * 80)))}, 0)`;
         } else if (vVal < 80) {
               const ratio = (80 - vVal) / (80 - 16);
               return `rgb(${Math.max(0, Math.min(255, Math.round(80 - ratio * 80)))}, ${Math.max(0, Math.min(255, Math.round(150 - ratio * 50)))}, 255)`;
         } else return "#999";
    };
    ...
    ['s', 'a', 't', 'b'].forEach(vKey => {
         ...
         voiceDisplay.id = `chords-display-${vKey}`;
         const currentVal = window.chordVoiceOverwrites[vKey];
         let displayText = "--";
         let bgColor = "white";
         let isBold = false;
         
         if (currentVal !== null) {
              const currentDyn = dynLabels.find(d => d.val === currentVal);
              displayText = currentDyn ? currentDyn.label : "--";
              bgColor = "#e8f5e9";
              isBold = true;
         }

         voiceDisplay.textContent = vKey.toUpperCase() + ": " + (displayText || "--");
         const bC = getDisplayColor(currentVal);
         const tC = (currentVal === 80 || currentVal === null) ? "#fff" : (currentVal > 80 ? "#331100" : "#ffffff");
         voiceDisplay.style.cssText = `display: inline-flex; align-items: center; justify-content: center; padding: 2px 6px; font-size: 11px; border: 1px solid #bbb; border-radius: 4px; background: ${bC}; font-weight: bold; min-width: 45px; height: 22px; flex-shrink: 0; color: ${tC}; cursor: default; user-select: none; font-family: monospace; transition: all 0.2s ease; border-shadow: 0 1px 2px rgba(0,0,0,0.1);`;
         
         mixerContainer.appendChild(voiceDisplay);
    });
    ...
    linkCheckbox.onchange = (e) => {
        ...
        if (e.target.checked) {
            ...
            linkLabelText.style.color = '#333';
            chordsPanel.classList.remove('linked-mode');
        }
        
        // Cambiar el color de fondo y de la cabecera para indicar visualmente la vinculación
        chordsPanel.style.setProperty('background', e.target.checked ? bodyColorLinked : bodyColorUnlinked, 'important');
        headerContainer.style.background = e.target.checked ? headerColorLinked : headerColorUnlinked;
        ...
        const prevCol = document.getElementById('chord-preview-column');
        if (prevCol) {
            prevCol.style.display = e.target.checked ? 'none' : 'block';
            // Trigger preview re-render when turning OFF to ensure colors are right
            if (!e.target.checked) {
                const staffCt = document.getElementById('chord-staff-container');
                const staged = staffCt?.dataset.stagedChord;
                if (staged && acordesNotas[staged]) {
        ...
        // Refrescar el editor MIDI para reflejar el cambio de modo Link
        if (window.refreshMidiEditorScore) window.refreshMidiEditorScore();
    };
    ...
    mainContainer.appendChild(mixerContainer); // Mezclador siempre bajo el header
    
    // Create Content Layout: Buttons Left, Preview Right
    const contentContainer = document.createElement('div');
    contentContainer.style.display = 'flex';
    contentContainer.style.flexDirection = 'row';
    contentContainer.style.width = '100%';
    contentContainer.style.boxSizing = 'border-box';
    contentContainer.style.padding = '0 0 15px 15px';
    
    const buttonsContainer = document.createElement('div');
    buttonsContainer.id = 'chords-buttons-container';
    
    const previewColumn = document.createElement('div');
    previewColumn.id = 'chord-preview-column';
    previewColumn.style.setProperty('width', '130px', 'important');
    previewColumn.style.setProperty('min-width', '130px', 'important');
    previewColumn.style.display = window.chordsMidiLinkEnabled ? 'none' : 'block';
    
    const previewName = document.createElement('div');
    previewName.id = 'chord-preview-name';
    previewName.style.cssText = 'display: flex; align-items: center; justify-content: space-between; gap: 4px; padding: 0 4px; box-sizing: border-box;';
    
    const nameLabel = document.createElement('span');
    nameLabel.id = 'chord-preview-name-label';
    nameLabel.textContent = 'Preview';
    nameLabel.style.flex = '1';
    
    // Pitch adjustment buttons
    if (window.chordsSopranoRefMidi === undefined) {
         const baseKey = (typeof keyinselecti !== 'undefined') ? keyinselecti : 0;
         window.chordsSopranoRefMidi = 72 + baseKey; 
    }

    const staffContainer = document.createElement('div');
    staffContainer.id = 'chord-staff-container';
    staffContainer.style.cursor = 'pointer';
    staffContainer.style.width = '120px'; // Para que el SVG interior no desborde
    staffContainer.title = (window.chordInsertModeEnabled ? "Click to insert chord as NEW measure" : "Click to apply to ALL voices (SATB)");

    const pitchControls = document.createElement('div');
    pitchControls.style.display = 'flex';
    pitchControls.style.flexDirection = 'column';
    pitchControls.style.gap = '1px';

    const btnUp = document.createElement('button');
    btnUp.textContent = '▲';
    btnUp.style.cssText = 'width: 14px; height: 10px; font-size: 8px; border: none; background: #555; color: white; cursor: pointer; border-radius: 2px 2px 0 0; line-height: 1; padding: 0;';
    btnUp.onclick = (e) => {
        e.preventDefault();
        e.stopPropagation();
        window.chordsSopranoRefMidi++;
        
        // Ensure preview reflects Soprano change: clear selection and force voice S
        if (window.selectedNoteIndices) window.selectedNoteIndices = [];
        const vSel = document.getElementById('voice-selector');
        if (vSel) vSel.value = 's';
        
        const currentChord = staffContainer.dataset.stagedChord;
        if (currentChord) {
            window.renderChordPreviewOnStaff(currentChord, acordesNotas[currentChord], staffContainer);
        }
    };

    const btnDown = document.createElement('button');
    btnDown.textContent = '▼';
    btnDown.style.cssText = 'width: 14px; height: 10px; font-size: 8px; border: none; background: #555; color: white; cursor: pointer; border-radius: 0 0 2px 2px; line-height: 1; padding: 0;';
    btnDown.onclick = (e) => {
        e.preventDefault();
        e.stopPropagation();
        window.chordsSopranoRefMidi--;
    ...
    pitchControls.appendChild(btnUp);
    pitchControls.appendChild(btnDown);

    previewName.appendChild(nameLabel);
    previewName.appendChild(pitchControls);
    
    previewColumn.appendChild(previewName);
    previewColumn.appendChild(staffContainer);
    
    // No optionsContainer appended here anymore, it's moved to header
    
    contentContainer.appendChild(buttonsContainer);
    contentContainer.appendChild(previewColumn);
    
    mainContainer.appendChild(contentContainer);
    chordsPanel.appendChild(mainContainer);

    // Commit Action: Clicking the staff applies it to the permanent score
    staffContainer.onclick = () => {
        ...
        if (chordName) {
            ...
            if (window.chordInsertModeEnabled) {
                ...
                // Insert Mode: Manual Step Insertion logic
                let measureIndex = window.currentEditingMeasureIndex;
                if (window.bdi && window.bdi.bar) {
                    // 0. Default target to last measure if none selected or empty
                    if (measureIndex < 0) measureIndex = window.bdi.bar.length - 1;
                    
                    // 1. Calculate SATB pitches for the chord
                    let rootMidi = 60;
                    const selectedIndices = window.selectedNoteIndices || [];
                    if (selectedIndices.length > 0 && window.currentFullMidiValues) {
                        rootMidi = Math.abs(window.currentFullMidiValues[selectedIndices[0]]) || 60;
                        ...
                        const vCode = voiceSelector ? voiceSelector.value : 's';
                        const baseKey = (typeof keyinselecti !== 'undefined') ? keyinselecti : 0;
                        const defaults = { 's': 72 + baseKey, 'a': 60 + baseKey, 't': 60 + baseKey, 'b': 48 + baseKey }; 
                        rootMidi = defaults[vCode] || (72 + baseKey);
                    }
                    
                    const intervals = acordesNotas[chordName];
                    const voiceSelector = document.getElementById('voice-selector');
                    const activeVoiceCode = voiceSelector ? voiceSelector.value : 's';
                    const satbResult = window.getSATBNotesForChord(rootMidi, intervals, activeVoiceCode);
                    const satb = satbResult.satb;

                    // 2. Create the fresh SATB measure object (FILL measure with ONE note)
                    // Get the single-note pattern for the current time signature from trilipi matrix
                    const defaultPattern = (window.trilipi && window.trilipi[1] && window.trilipi[1][0]) ? [...window.trilipi[1][0]] : [1];
                    ...
                    // 3. Manual Insertion into BDI (Inserting BEFORE the current selection)
                    window.bdi.bar.splice(measureIndex, 0, newMeasure);
                    window.rebuildRecordi();
                    if (typeof window.applyTextLayer === 'function') window.applyTextLayer();
                    ...
                    // 5. Finalize the view (Select the note)
                    setTimeout(() => {
                        window.selectedNoteIndices = [0];
                        const singleInput = document.getElementById('midi-single-input');
                        if (singleInput) {
                ...
                // Traditional Mode: Modify current measure
                window.applyChordToSelectedNotes(chordName);
            }
            
            // Visual feedback
            staffContainer.style.background = '#e8f5e9';
            setTimeout(() => { staffContainer.style.background = '#fff'; }, 300);
        }
    };

    // Initialize reference pitch if not set
    if (window.chordsSopranoRefMidi === undefined) {
        const baseKey = (typeof keyinselecti !== 'undefined') ? keyinselecti : 0;
        window.chordsSopranoRefMidi = 72 + baseKey; 
    }

    // MUSICOLI: Determine default chord based on scale mode for initial highlight
    const scaleModeSelect = document.getElementById('scale-mode');
    const scaleMode = scaleModeSelect ? scaleModeSelect.value : 'Major';
    let defaultChord = (scaleMode === 'Major' || scaleMode === 'Lidio' || scaleMode === 'Mixolidio') ? 'Maj' : 'min';
    if (!acordesNotas[defaultChord]) defaultChord = Object.keys(acordesNotas)[0];

    // Populate buttons
    Object.keys(acordesNotas).forEach(name => {
        const btn = document.createElement('button');
        btn.className = 'chord-btn theme-btn';
        if (name === defaultChord) btn.classList.add('active');
        ...
        btn.onclick = (e) => {
            ...
            // MUSICOLI: Manage active highlight
            buttonsContainer.querySelectorAll('.chord-btn').forEach(b => b.classList.remove('active'));
            btn.classList.add('active');
            ...
            if (selectedIndices.length > 0 && window.currentFullMidiValues) {
                ...
                // Return to previous logic: Use Key-based defaults but allow reference override
                const baseKey = (typeof keyinselecti !== 'undefined') ? keyinselecti : 0;
                const defaults = { 
                    's': window.chordsSopranoRefMidi, 
                    'a': 60 + baseKey, 
                    't': 60 + baseKey, 
                    'b': 48 + baseKey 
                };
                rootMidi = defaults[activeVoiceCode] || window.chordsSopranoRefMidi;
            }
            
            // 2. Play audition
            window.auditionSATBChord(rootMidi, intervals, activeVoiceCode);

            // 3. Stage in mini-staff
    ...
    if (defaultChord) {
        // We ensure Soprano Reference is used for initial preview
        const voiceSelector = document.getElementById('voice-selector');
        const activeVoiceCode = voiceSelector ? voiceSelector.value : 's';
        const baseKey = (typeof keyinselecti !== 'undefined') ? keyinselecti : 0;
        
        let rootMidi;
        if (activeVoiceCode === 's') {
            rootMidi = window.chordsSopranoRefMidi || (72 + baseKey);
        } else {
            const defaults = { 'a': 60+baseKey, 't': 60+baseKey, 'b': 48+baseKey };
            rootMidi = defaults[activeVoiceCode] || 72;
        }

        window.renderChordPreviewOnStaff(defaultChord, acordesNotas[defaultChord], staffContainer);
        staffContainer.dataset.stagedChord = defaultChord;
    }
};
...
window.applyChordToSelectedNotes = function(chordType) {
    ...
    let singleInput = document.getElementById('midi-single-input');
    let currentMidiVals = [];
    if (singleInput && singleInput.value) {
        currentMidiVals = singleInput.value.trim().split(/\s+/).map(v => parseInt(v)).filter(v => !isNaN(v));
    }
    
    // Fallback: If input is empty (e.g., during transition), use BDI state directly
    if (currentMidiVals.length === 0) {
        const vCode = document.getElementById('voice-selector')?.value || 's';
        const vData = measure.voci.find(v => v.nami === vCode);
        currentMidiVals = vData ? [...vData.nimidi] : [];
    }
    
    if (currentMidiVals.length === 0) {
        console.warn("Musicoli: No MIDI values found to anchor the chord.");
        return;
    }

    // console.log(`Applying ${chordType} to measure ${measureIndex}, indices:`, selectedIndices);
    ...
    let curPos = 0;
    activeVoiceRhythms.forEach(t => {
        activeVoiceTimis.push(curPos);
        curPos += Math.abs(t);
    });
    ...
    measure.voci.forEach(v => {
        ...
        if (!v.timis || v.timis.length === 0 || v.timis.length !== v.tipis.length) {
            let voiceTimis = [];
            let vPos = 0;
            v.tipis.forEach(t => {
                voiceTimis.push(vPos);
    ...
    if (updatedVoice) {
        // Find input again to ensure we have the one in the NEW editor if it just opened
        const finalInput = document.getElementById('midi-single-input');
        if (finalInput) {
            finalInput.value = updatedVoice.nimidi.join(' ');
            
            // Fire input event to trigger notations/score update in the editor modal
            finalInput.dispatchEvent(new Event('input', { bubbles: true }));
        }
    }

    // Force full score preview refresh (playback and visualizer)
    if (typeof rebuildRecordi === 'function') {
        rebuildRecordi();
    }

    // Update the main matrix (the top track grid) to reflect new colors
    if (typeof renderVisualTracks === 'function') {
        renderVisualTracks();
    }
};
...
/**
 * MUSICOLI: MIDI STAFF VISIBILITY CONTROLLER (Global)
 * Toggles the visibility of the midi-visualizer staff.
 * @param {boolean} [visible] - Optional forced visibility state.
 */
window.toggleMidiStaff = function(visible) {
    const staff = document.getElementById('staffi');
    const toggle = document.getElementById('score-viz-toggle');
    
    if (staff) {
        // If visible is not provided, toggle the current state
        const targetVisible = (typeof visible === 'boolean') ? visible : (staff.style.display === 'none');
        staff.style.display = targetVisible ? 'block' : 'none';
        
        // Always sync the check box if it exists
        if (toggle) toggle.checked = targetVisible;
    }
};
